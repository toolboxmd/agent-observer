...
- Failure class: observed category of a failed production attempt from explicit terminal, stage, rc and proof_class only: timeout, stall, provider, infrastructure, implementation, verification, or unknown when classification evidence is missing. Context pressure is provider. Supervisor rc124 without proof outcome is infrastructure; executed proof rc124 stays timeout; legacy rows without rc keep terminal-only behavior. Router reason describes why an invocation was launched and never classifies a later failure.
