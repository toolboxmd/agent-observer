...
- Recovery: for one failed execution, the failure-to-next-attempt-start duration inside the same compatible identity (same Router request_id or same known non-shared session), the first subsequent completed progress measured at its end when observable, and the outcome with repeated failed counts in that chain only. A new attempt starting alone is not successful recovery; unresolved recovery stays active or unknown. Attempts from another request or shared/unknown sessions never pair.
